...
contract BtcBobMarketPlace {
    ...
    /**
     * @dev Mapping to store BTC to USDC (or other ERC20) swap orders based on their unique identifiers.
    ...
    /**
     * @dev Mapping to store ordinal sell orders for swapping BTC to USDC (or other ERC20) based on their unique identifiers.
     ...
     * not just USDC (Tether). Make sure to set this to the appropriate ERC-20 contract address.
     */
    IERC20 public usdcContractAddress;

    /**
     * @dev Counter for generating unique identifiers for BTC to USDC swap orders.
     * The `nextBtcOrderId` is incremented each time a new BTC to USDC swap order is created,
    ...
    /**
     * @dev Struct representing a BTC to USDC swap order.
     */
    struct BtcSellOrder {
    ...
    /**
     * @dev Struct representing an ordinal sell order for swapping Ordinal to USDC.
     */
     ...
     *   - Transfers the locked USDC amount to the original seller.
        ...
        public
    {
        ...
        // Emit an event indicating the successful completion of the BTC to USDC swap
