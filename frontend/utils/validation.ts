...
export const isFormDisabled = (form: any, shouldBeDirty = true): boolean => !form.isValid || (shouldBeDirty && !form.dirty);
...
export const isValidBTCAddress = (address: string): boolean => {
  ...
  throw new Error(`Invalid bitcoin network configured: ${BITCOIN_NETWORK}. Valid values are: testnet | regtest | mainnet.`);
};
